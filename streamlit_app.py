...
def simulate(v0, m_g, d_mm, theta_deg, phi_deg, h0, target_dh, wx, wz, cd0, cl0):
    ...
    while y >= (target_dh - 5.0) and x <= 165:
        vrx, vry, vrz = vx - wx, vy, vz - wz
        v = math.sqrt(vrx**2 + vry**2 + vrz**2)
        if v < 1e-6: break
        
        Fd = 0.5 * rho * (cd0 * (1 + 0.15 * (v/60)**2)) * A * v**2
...
with st.sidebar:
    ...
    m_g = st.number_input("화살 무게 (g)", 15.0, 40.0, 20.0)
    theta_deg = st.number_input("수직 발사각 (°)", 0.0, 45.0, 13.0)
    phi_deg = st.number_input("좌우 발사각 (°)", -5.0, 5.0, 0.0)
    h0 = st.number_input("발시 높이 (m)", 0.0, 5.0, 1.5)
    
    st.markdown("---")
    st.subheader("🌬️ 환경 변수")
    wx = st.slider("순풍(+) / 역풍(-) (m/s)", -15.0, 15.0, 0.0)
    wz = st.slider("측풍 (L:-, R:+) (m/s)", -10.0, 10.0, 0.0)
    target_dh = st.number_input("과녁 상대높이 (m)", -10.0, 10.0, 0.0)

# 시뮬레이션 실행
xs, ys, zs, vs = simulate(v0, m_g, 8.0, theta_deg, phi_deg, h0, target_dh, wx, wz, 0.9, 0.05)
...
st.sidebar.download_button(
    label="📥 시뮬레이션 데이터(CSV) 다운로드",
    data=csv_data,
    file_name='jumong_trajectory.csv',
    mime='text/csv',
)
...
with top_col1:
    ...
    ax_wind.set_xlabel("순풍/역풍"); ax_wind.set_ylabel("측풍")
...
# 1. 측면 탄도
ax1.plot(xs, ys, color='#2ecc71', lw=2)
ax1.plot([t_base, t_base + t_h*math.sin(tilt)], [target_dh, target_dh + t_h*math.cos(tilt)], 'r-', lw=5, label="과녁")
ax1.set_title("측면 비행 탄도 (Side View)"); ax1.grid(True)

# 2. 상면 궤적
ax2.plot(xs, zs, color='#e67e22', lw=2)
ax2.axvline(x=t_base, color='red', linestyle='--', alpha=0.5)
ax2.set_ylim(-3, 3)
ax2.set_title("상면 비행 궤적 (Top View)"); ax2.grid(True)
...
ax3.set_title("과녁 정면 적중 지점 (Front View)"); ax3.grid(True)
