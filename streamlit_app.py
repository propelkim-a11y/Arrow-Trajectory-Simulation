...
# 상단 레이아웃 (윈드벡터 극소화)
top_col1, top_col2 = st.columns([1, 4]) # 윈드벡터 영역을 더 좁게 할당
with top_col1:
    # figsize를 (1.0, 1.0)으로 극소화
    fig_wind, ax_wind = plt.subplots(figsize=(1.0, 1.0))
    ax_wind.quiver(0, 0, wx, wz, angles='xy', scale_units='xy', scale=1, color='#3498db', width=0.15)
    ax_wind.set_xlim(-15, 15); ax_wind.set_ylim(-15, 15)
    # 제목과 라벨 제거하여 공간 확보
    ax_wind.set_title("Wind", fontsize=7, pad=2)
    ax_wind.set_xticks([]); ax_wind.set_yticks([]) # 눈금 제거
...
if 'y' in hit_data:
    ax3.plot(hit_data['z'], hit_data['y'], 'ro' if hit_data['hit'] else 'kx', markersize=5)
