...
with st.sidebar:
    ...
    # 그래프 높이 조절 슬라이더 추가 (최소 6, 최대 20)
    graph_height = st.slider("그래프 세로 높이 (Total Height)", 6, 20, 10)
...
# 그래프 시각화 (슬라이더 반영)
# =========================
# figsize=(10, graph_height)를 통해 세로 크기를 동적으로 변경합니다.
...
ax1.set_ylim(-2, max(ys)*1.2 if len(ys)>0 else 15) # 궤적 높이에 맞게 자동 조정
