...
# 버전 및 설정
# =========================
VERSION = "v1.4.2 (2026-01-14)"
st.set_page_config(page_title="주몽정 - 국궁 시뮬레이터", layout="wide")

def set_korean_font():
    # OS별 한글 폰트 설정 (그래프 내부 한글 깨짐 방지용)
    paths = [
...
def simulate(v0, m_g, d_mm, theta_deg, phi_deg, h0, target_dh, wx, wz, cd0, cl0):
    ...
    while y >= (target_dh - 5.0) and x <= 165:
        ...
        # 공기저항력 $F_d = \frac{1}{2} \rho v^2 C_d A$
        Fd = 0.5 * rho * (cd0 * (1 + 0.15 * (v/60)**2)) * A * v**2
        # 양력 $F_l = \frac{1}{2} \rho v^2 C_l A$
...
with st.sidebar:
    ...
    cd0 = st.slider("항력계수 (Cd)", 0.1, 1.5, 0.9, help="공기 저항 계수입니다. 높을수록 비거리가 짧아집니다.")
    cl0 = st.slider("양력계수 (Cl)", 0.0, 0.5, 0.05, help="화살이 비행 중 떠오르는 힘의 계수입니다.")
...
df = pd.DataFrame({
    ...
    'Height_Y': ys, 
    'Lateral_Z': zs,
    'Vel_X': [v[0] for v in vs], # 튜플 인덱싱 에러 수정
...
st.sidebar.download_button(label="📥 시뮬레이션 데이터(CSV) 다운로드", data=csv_data, file_name='jumong_trajectory.csv', mime='text/csv')
...
# 사대(0,0)에서 과녁 바닥(145, target_dh)까지 직선 추가
