...
with st.sidebar:
    ...
    m_g = st.number_input("화살 무게 (g)", 15.0, 40.0, 26.25) # 초기값 설정
    theta_deg = st.number_input("수직 발사각 (°)", 0.0, 45.0, 13.0)
    phi_deg = st.number_input("좌우 발사각 (°)", -5.0, 5.0, -0.5) # 초기값 설정
    ...
    wx = st.slider("순풍(+) / 역풍(-) (m/s)", -15.0, 15.0, 11.0) # 초기값 설정
    wz = st.slider("측풍 (L:-, R:+) (m/s)", -10.0, 10.0, 8.0) # 초기값 설정
    target_dh = st.number_input("과녁 상대높이 (m)", -10.0, 10.0, 2.0) # 초기값 설정

# 시뮬레이션 실행
xs, ys, zs, vs = simulate(v0, m_g, 8.0, theta_deg, phi_deg, h0, target_dh, wx, wz, cd0, cl0)

# =========================
# 결과 분석 및 CSV 생성 (Pandas 에러 수정)
# =========================
df = pd.DataFrame({
    'Distance_X': xs, 
    'Height_Y': ys, 
