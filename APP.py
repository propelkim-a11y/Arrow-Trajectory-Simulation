...
X_MAX = 160  # 🔧 Side / Top View 공통 x축 범위
...
ax2.set_xlim(0, X_MAX)   # 🔧 Side View와 동일
ax2.set_ylim(-2, 2)
...
if hit:
    ax3.plot(hit[2], hit[1] - target_h, "ro", ms=10)
    status = "HIT" if (-1 <= hit[2] <= 1 and
                       0 <= hit[1] - target_h <= TARGET_HEIGHT) else "MISS"
