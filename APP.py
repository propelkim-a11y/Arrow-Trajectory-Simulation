...
def simulate(v0, m_g, d_mm,
    ...
    # target plane (x-y plane tilt)
    def plane_f(x, y):
        return y - (target_dh + (1 / math.tan(alpha)) * (x - TARGET_X))
...
yt = [target_dh + (1 / math.tan(math.radians(15))) * (x - TARGET_X) for x in xt]
...
if hit:
    ...
    inside = (-TARGET_WIDTH/2 <= hz <= TARGET_WIDTH/2) and (0 <= rel_y <= TARGET_HEIGHT)
