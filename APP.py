...
def simulate(v0, m_g, d_mm,
    ...
    # World coordinates: shooter floor = target floor = y = 0
    x, y, z = 0.0, h0, 0.0
    xs, ys, zs = [x], [y], [z]

    # Target plane: vertical plane tilted backward
    def plane_f(x, y):
...
# Target plane (longer segment so height change is visible)
xt = [TARGET_X - 2.0, TARGET_X + 2.0]
yt = [
...
# --- Front View (world Y coordinates) ---
ax3 = fig.add_subplot(gs[2])
ax3.add_patch(
    plt.Rectangle(
        (-TARGET_WIDTH / 2, target_h),
        TARGET_WIDTH,
        TARGET_HEIGHT,
