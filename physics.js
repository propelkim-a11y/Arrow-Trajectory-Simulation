// =========================================================================
// [사방 정밀 눈금선 완비] physics.js - 전체 소스 코드
// =========================================================================
...
function drawScene() {
    ...
    // 우측 및 상단 테두리에 맞추어 눈금 텍스트가 잘리지 않도록 마진 여백 재조정
    const RIGHT_MARGIN = 20;
    const TOP_MARGIN = 15;
    
    const availW = dprWidth - ORIGIN_X_OFFSET - RIGHT_MARGIN; 
    const availH = dprHeight - GROUND_Y_OFFSET - TOP_MARGIN;
...
const frontScaleZ = (dprWidth - ORIGIN_X_OFFSET - RIGHT_MARGIN) / (MAX_WORLD_Z * 2); 
...
function toScreen(pX, pY, pZ) {
    ...
    if (currentView === 'front') return { x: ORIGIN_X_OFFSET + (dprWidth - ORIGIN_X_OFFSET - RIGHT_MARGIN) / 2 + (pZ * frontScaleZ), y: dprHeight - GROUND_Y_OFFSET - (pY * frontScaleY) };
...
if (currentView === 'side') {
    const borderRightX = ORIGIN_X_OFFSET + (MAX_WORLD_X * scaleX);
    const borderTopY = dprHeight - GROUND_Y_OFFSET - (MAX_WORLD_Y * scaleY);
    
    // [측면도] 가로 눈금선 및 보조선 (하단 및 💡상단 대칭 배치)
    for (let xMeters = 0; xMeters <= MAX_WORLD_X; xMeters += 20) {
        let scrX = ORIGIN_X_OFFSET + (xMeters * scaleX);
        ctx.beginPath(); 
        ctx.moveTo(scrX, borderTopY); 
        ctx.lineTo(scrX, dprHeight - GROUND_Y_OFFSET); 
        ctx.stroke();
        ctx.textAlign = 'center';
        ctx.fillText(xMeters + 'm', scrX, dprHeight - GROUND_Y_OFFSET + 14); // 하단 라벨
    }
    
    // [측면도] 세로 눈금선 및 보조선 (좌측 및 💡우측 대칭 배치)
    for (let yMeters = 0; yMeters <= MAX_WORLD_Y; yMeters += 5) {
        ...
        ctx.lineTo(borderRightX, scrY); 
        ctx.stroke();
        ctx.textAlign = 'right';
        ctx.fillText(yMeters + 'm', ORIGIN_X_OFFSET - 5, scrY + 3); // 좌측 라벨
    }
    
    // 💡 [측면도] 깔끔한 사방 외곽 테두리선 강조 완료
    ctx.strokeStyle = '#1d1d1f'; 
    ctx.lineWidth = 2;
    
    ctx.beginPath(); 
    ctx.moveTo(ORIGIN_X_OFFSET, dprHeight - GROUND_Y_OFFSET); 
    ctx.lineTo(borderRightX, dprHeight - GROUND_Y_OFFSET); 
    ctx.stroke(); // 하단축
    
    ctx.beginPath(); 
    ctx.moveTo(ORIGIN_X_OFFSET, borderTopY); 
    ctx.lineTo(ORIGIN_X_OFFSET, dprHeight - GROUND_Y_OFFSET); 
    ctx.stroke(); // 좌측축
    
    ctx.beginPath(); 
    ctx.moveTo(ORIGIN_X_OFFSET, borderTopY); 
    ctx.lineTo(borderRightX, borderTopY); 
    ctx.stroke(); // 상단 반대축
    
    ctx.beginPath(); 
    ctx.moveTo(borderRightX, borderTopY); 
    ctx.lineTo(borderRightX, dprHeight - GROUND_Y_OFFSET); 
    ctx.stroke(); // 우측 반대축
    ...
    const centerX = ORIGIN_X_OFFSET + (dprWidth - ORIGIN_X_OFFSET - RIGHT_MARGIN) / 2;
    const borderLeftX = centerX + (-MAX_WORLD_Z * frontScaleZ);
    const borderRightX = centerX + (MAX_WORLD_Z * frontScaleZ);
    const borderTopY = dprHeight - GROUND_Y_OFFSET - (MAX_WORLD_Y * frontScaleY);
    
    // [정면도] 가로 좌우 눈금선 (하단 및 💡상단 대칭 배치)
    for (let zMeters = -MAX_WORLD_Z; zMeters <= MAX_WORLD_Z; zMeters += 5) {
        ...
        ctx.stroke();
        ctx.textAlign = 'center';
        ctx.fillText(zMeters === 0 ? '0m' : zMeters + 'm', scrX, dprHeight - GROUND_Y_OFFSET + 14); // 하단 라벨
    }
    
    // [정면도] 세로 높이 눈금선 (좌측 및 💡우측 대칭 배치)
    for (let yMeters = 0; yMeters <= MAX_WORLD_Y; yMeters += 5) {
        let scrY = dprHeight - GROUND_Y_OFFSET - (yMeters * frontScaleY);
        ctx.beginPath(); 
        ctx.moveTo(borderLeftX, scrY); 
        ctx.lineTo(borderRightX, scrY); 
        ctx.stroke();
        ctx.textAlign = 'right';
        ctx.fillText(yMeters + 'm', borderLeftX - 5, scrY + 3); // 좌측 라벨
    }
}
    // 💡 [정면도] 사방 사각형 외곽선 및 센터 중앙 전진 수직축 강조 완료
    ctx.strokeStyle = '#1d1d1f'; 
    ctx.lineWidth = 2;

    ctx.beginPath(); 
    ctx.moveTo(borderLeftX, dprHeight - GROUND_Y_OFFSET); 
    ctx.lineTo(borderRightX, dprHeight - GROUND_Y_OFFSET); 
    ctx.stroke(); // 하단축

    ctx.beginPath(); 
    ctx.moveTo(borderLeftX, borderTopY); 
    ctx.lineTo(borderLeftX, dprHeight - GROUND_Y_OFFSET); 
    ctx.stroke(); // 좌측축
    ...
    ctx.beginPath(); 
    ctx.moveTo(0, tBottomY); 
    ctx.lineTo(dprWidth, tBottomY); 
    ctx.stroke();
}
