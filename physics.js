...
// 초기화 이벤트 리스너 바인딩 안전성 개선
window.addEventListener('load', () => {
    canvas = document.getElementById('simCanvas');
    if (canvas) {
        ctx = canvas.getContext('2d');
        resizeCanvas(); // 1. 먼저 뷰포트 크기를 완벽히 맞추고
        window.addEventListener('resize', resizeCanvas);
        
        // 2. ui.js 내부 DOMContentLoaded 타이밍과의 경합을 피해 궤적 연산 실행
        setTimeout(() => {
            fireArrow();
        }, 100); 
    }
...
function drawScene() {
  ...
  // ============================================================================
// [Physics Core - Part 2] 3 시점별 차원 좌표축 및 과녁 보조선 렌더링 엔진
// ============================================================================

// ... (상단 drawScene 함수 도입부 생략) ...
...
if (viewMode === 'side') {
    ...
    // (X축) 메인 사대 지면 바닥선 축 기본 물리 기준선
    ctx.strokeStyle = '#1d1d1f';
    ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(startX - 20, groundY); ctx.lineTo(endX + 20, groundY); ctx.stroke();

    // (Z축) 수직 고도 탐지선 축 기본 물리 기준선
    ctx.beginPath(); ctx.moveTo(startX, groundY + 10); ctx.lineTo(startX, topY - 20); ctx.stroke();

    // 물리 좌표축 방향 레이블 네임텍스트 매핑
    ctx.fillStyle = '#86868b';
    ctx.textAlign = 'left';
    ctx.fillText(' X (m)', endX + 25, groundY);
    ctx.textAlign = 'center';
    ctx.fillText(' Z (m)', startX, topY - 30);

    // [완전 복구] (0m ~ 160m) 수평 거리 눈금 스케일 상시 고정 주입 구간
    const distances = [0, 20, 40, 60, 80, 100, 120, 145, 160]; // 🚨 빈 배열에 고정 스케일 데이터 주입
    distances.forEach(d => {
    ...
    // [완전 복구] (0m ~ 40m) 상방 수직 높이 눈금 스케일 상시 고정 주입 구간
    const heights = [0, 10, 20, 30, 40]; // 🚨 빈 배열에 고정 고도 데이터 주입
    ...
    // ... (이후 145m 과녁 및 십자 지표 마커 연출부 코드는 동일합니다) ...
    ...
    // [완전 복구] 평면 기준 종방향 거리 스케일 단위 눈금 상시 고정 투영 (0m ~ 160m 구간)
      // [완전 복구] 평면 기준 종방향 거리 스케일 단위 눈금 상시 고정 투영 구간 (0m ~ 160m 구간)
    const distances = [0, 20, 40, 60, 80, 100, 120, 145, 160]; // 🚨 데이터 복구 완료
    ctx.textAlign = 'center';
    distances.forEach(d => {
        const tickX = startX + (d / 160) * (canvas.width * 0.8);
