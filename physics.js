...
window.addEventListener('load', () => {
    ...
    if (canvas) {
        ...
        fireArrow(); // 초기 화면 엔진 로드 즉시 기본 변수 기반 가상 비행 탄도 렌더링
    }
});
...
function fireArrow() {
    ...
    // 2. 물리 표준 단위계 변환 연산 오류 방어 수치 정형화
...
// ============================================================================
// [Physics Core - Part 2] 시점별 차원 좌표축 및 과녁 보조선 렌더링 엔진 (Fix Completed)
// ============================================================================
...
function drawScene() {
    ...
    // --------------------------------------------------------------------------
    // 1. (Grid Layer) 구조 배경 베이스 그리드 레이어
    // --------------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------------
    // 2. (Axis Layer) 프리미엄 계측 좌표축 및 과녁 보조선 레이어
    // --------------------------------------------------------------------------
    ...
    if (viewMode === 'side') {
        ...
        // [정밀 복구 수행] 수평 거리 눈금 수치 데이터 명확히 주입 완료
        const distances =;
        distances.forEach(d => {
        ...
        // [정밀 복구 수행] 상방 수직 높이 눈금 수치 데이터 명확히 주입 완료
        const heights =;
        ...
        // 145m 핵심 크로스 보조선 정밀 국궁 과녁 십자 지표 마커 타겟 연출
        ...
        // 정중앙 수직 중심 영점 라인 가이드선 오프셋 기준 지표선 (Y = 0m)
        ...
        // [정밀 복구 수행] 평면 기준 종방향 거리 눈금 수치 데이터 명확히 주입 완료
        const distances =;
        ctx.textAlign = 'center';
        distances.forEach(d => {
            const tickX = startX + (d / 160) * (canvas.width * 0.8);
    ...
    // --------------------------------------------------------------------------
    // 3. (Trajectory Arrow Layer) 최상단 최우선 순위 비행 탄도 궤적 렌더링 레이어
    // --------------------------------------------------------------------------
