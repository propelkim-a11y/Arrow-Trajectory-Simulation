// =========================================================================
// [최종 완성본] physics.js - 전체 소스 코드 (모든 시점 과녁 증발 버그 해결)
// =========================================================================
...
const MAX_WORLD_Y = 30;    // 최대 높이 30m (과녁 고도가 높아지면 이 상수를 늘려야 뷰에 잡힙니다)
...
function getDynamicTargetGeometry() {
    ...
    // 입력창이 비어있거나 이상값이 들어왔을 때 기본값 보장 (국궁 기본 바닥 고도 보통 0~1.5m 사이)
...
function drawScene() {
  ...
  // [중요 보정] 과녁이 박스 내부에 알맞고 거대하게 잡히도록 뷰포트 배율을 최적화(전체화면 대비 25% 크기)
  ...
  function toScreen(pX, pY, pZ) {
    ...
    if (currentView === 'target') {
      // [중요 보정] 과녁이 상단 밖으로 가출하는 현상을 막기 위해 y축 정중앙 오프셋을 화면의 딱 65% 지점(하부 쪽)으로 가라앉힘
  ...
  if (currentView === 'side') {
    ...
    ctx.fillStyle = '#1d1d1f'; ctx.fillRect(leftX + w * 0.1, topY + h * 0.08, w * 0.8, topBarH);
    ...
    const topBarH = h * 0.15;
    ctx.fillStyle = '#1d1d1f'; ctx.fillRect(tLeftX + w * 0.1, tTopY + h * 0.08, w * 0.8, topBarH);
